# -*- coding: utf-8 -*-
"""
Created on Tue May 28 13:55:10 2024

@author: rohdo & James Schombert
see original at https://astro.ucla.edu/~wright/CC.python
"""
...
def lumDistLCDM(z):
		"""gets the luminosity distance (in meters) as a function of the redshift z 
		for a flat LCDM with OmegaM = 0.31, OmegaLambda = 0.69 and H0 = 68
		"""
		
		return cosmoCalc([z, 68, 0.31, 0.69]) * 3.086e+22
		
def cosmoCalc(cosmology):
		try:
			if cosmology[0] == '-h':
		...
		Options:	 -h for this message 
			...
			if cosmology[0] == '-v':
				verbose=1
				length=len(cosmology)-1
			else:
				verbose=0
				length=len(cosmology)
		
		# if no values, assume Benchmark Model, input is z
			if length == 1:
				if float(cosmology[verbose]) > 100:
					z=float(cosmology[verbose])/299790.		# velocity to redshift
				else:
					z=float(cosmology[verbose])						# redshift
		...
		# if one value, assume Benchmark Model with given Ho
				...
				z=float(cosmology[verbose])		# redshift
				H0 = float(cosmology[1+verbose]) # Hubble constant
				WM = 0.3												# Omega(matter)
				WV = 1.0 - WM - 0.4165/(H0*H0)	# Omega(vacuum) or lambda
		...
		# if Univ is Open, use Ho, Wm and set Wv to 0.
				...
				z=float(cosmology[verbose])		# redshift
				H0 = float(cosmology[1+verbose]) # Hubble constant
				WM = float(cosmology[2+verbose]) # Omega(matter)
		...
		# if Univ is General, use Ho, Wm and given Wv
				...
				H0 = float(cosmology[1+verbose]) # Hubble constant
				WM = float(cosmology[2+verbose]) # Omega(matter)
				WV = float(cosmology[3+verbose]) # Omega(vacuum) or lambda
